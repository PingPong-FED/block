...
import Reattempt, { ReattemptOptions } from 'reattempt'
...
const defaultRetryConfig: ReattemptOptions = {
  times: 1
}
/**
 * 请求
...
export default function request (
  ...
  let req: any = async () => await block.instance(requestConf)
  // 重试机制
  if (config && config.retry) {
    req = Reattempt.run(config.retry, req as any)
  }
  // 发送请求
  return req
}
