 ...
 * api 单模块实例
 */
export interface RawModule extends ModuleConfig {
  api: {
...
export default class Module {
  ...
  // 外部传入的block实例
  public blockPri: Block | undefined
  constructor (
    rawModule: RawModule,
    parentName: string,
    blockPri?: Block | undefined
  ) {
    this.baseURL = ''
    this.interceptors = {}
    this.apisConfig = {}
  ...
  private init (rawModule: RawModule) {
    ...
    this.createNew =
      this.addInterceptor || objectOwnedAttr(config) || !!this.blockPri
    ...
    if (this.createNew) {
      this.block = this.blockSelect(config, request, response)
    }
    this.apiCollect(api)
  }
  ...
  private blockSelect (
    ...
    // 全局的block实例
    let block = this.blockPri || cache.block
    if (objectOwnedAttr(config)) {
      block = new Block(config)
      block.addInts(
        request || response ? { request, response } : cache.interceptor
      )
    }
    return block
  }
